...
class UI_Logic_Window(QtWidgets.QMainWindow):
    ...
    def init_slots(self):
        ...
        # 实时监测槽绑定
        self.ui.online_camer.clicked.connect(self.online_camera_open)
    ...
    def online_camera_open(self):
        ...
        if mode == "video":
            ...
            while(True):
                ...
                # # # 将OpenCV的BGR图像转换为QImage的RGB图像  
                # height, width, channel = frame.shape  
                # bytes_per_line = channel * width  
                # frame = QImage(frame.data, width, height, bytes_per_line, QImage.Format_BGR888)
                # self.ui.label.setPixmap(QtGui.QPixmap.fromImage(frame))
                # self.ui.label.setScaledContents(True) # 设置图像自适应界面大小

                # cv2.imshow("online_detetion",frame) 
...
if __name__ == '__main__':
    stylesheet = """
            UI_Logic_Window {
                background-image: url("ui_img/background.png");
